// tank_shooter_v3_5.cpp
// Tank Shooter v3.5 - Ultimate Multi-Enemy & Explosion Edition
...
enum EnemyType { NORMAL, FAST, STRONG, BOUNCER, ZIGZAG };
struct Enemy {
  int x, y, hp, dir;
  EnemyType type;
...
void drawEnemyShape(vector<string> &scr, const Enemy &e) {
  ...
  switch (e.type) {
    ...
    case STRONG:
      ch = '@'; color = 31;
      parts = {{0,0},{-1,0},{1,0},{0,1},{-1,1},{1,1}};
      break;
    case BOUNCER:
      ch = '&'; color = 93;
      parts = {{0,0},{-1,0},{1,0},{0,1}};
      break;
    case ZIGZAG:
      ch = 'Z'; color = 96;
      parts = {{0,0},{1,1},{-1,1}};
  ...
  for (auto &p : parts) {
    ...
    if (nx>=1 && nx<WIDTH-1 && ny>=1 && ny<HEIGHT-1) scr[ny][nx] = ch;
  }
}
...
void spawnEnemiesByLevel() {
  ...
  for (int i=0;i<cnt;i++) {
    ...
    else if (r < 80) t = STRONG;
    else if (r < 90) t = BOUNCER;
    else t = ZIGZAG;
    enemies.emplace_back(2 + rand() % (WIDTH - 6), 2, t);
  }
}
...
void updateGameLogic() {
  ...
  // Enemy move – gi?m t?c d? t?ng th?
  if (tickCount % max(2, 10 - level / 2) == 0) {
    for (auto &e : enemies) {
      switch (e.type) {
        case FAST: 
          e.y += 1;      // Tru?c là 2 ? gi?m còn 1
          break;
        case STRONG:
          e.y += 1;      // Gi? nguyên
          break;
        ...
        case ZIGZAG:
          e.y += 1;
          if (tickCount % 12 == 0) e.dir *= -1;  // Zigzag d?i hu?ng ch?m hon
          e.x += e.dir;
          break;
      }
    }
...
void showInstructions() {
  ...
  cout << "- Choose tank type wisely for your play style.\n";
  cout << "- Enemies:\n";
  cout << "   # Normal   - balanced\n";
  cout << "   > Fast     - quick but weak\n";
  cout << "   @ Strong   - slow but tough\n";
...
void runGameLoop() {
  ...
  score = tickCount = level = 1; enemySpawnRate = START_ENEMY_RATE;
  running = true;
